...
fn shallowVelocityYStep2(@builtin(global_invocation_id) globalIdx: vec3u) {
    ...
    */
    ...
    let heightStaggeredUp = max(textureLoad(heightIn, vec2i(ix , clampI(iy + i32(upWindHeight(velYUp)), 0, i32(size.y) - 1))).x, H_EPS);
    let heightCenter = max(textureLoad(heightIn, vec2i(ix, iy)).x, H_EPS);
    let heightUp = max(textureLoad(heightIn, vec2i(ix, clampI(iy + 1, 0, i32(size.y) - 1))).x, H_EPS);
