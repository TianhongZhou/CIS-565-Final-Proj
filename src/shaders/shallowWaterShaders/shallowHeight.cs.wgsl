...
fn shallowHeight(@builtin(global_invocation_id) globalIdx: vec3u) {
    ...
    let terrainRight = textureLoad(terrainTexture, heightRightIndex, 0).x;
    let terrainLeft = textureLoad(terrainTexture, heightLeftIndex, 0).x;
    let heightRight = max(textureLoad(heightIn, heightRightIndex).x + terrainRight, H_EPS);
    let heightLeft = max(textureLoad(heightIn, heightLeftIndex).x + terrainLeft, H_EPS);
    ...
    let terrainUp = textureLoad(terrainTexture, heightUpIndex, 0).x;
    let terrainDown = textureLoad(terrainTexture, heightDownIndex, 0).x;
    let heightUp = max(textureLoad(heightIn, heightUpIndex).x + terrainUp, H_EPS);
    let heightDown = max(textureLoad(heightIn, heightDownIndex).x + terrainDown, H_EPS);
